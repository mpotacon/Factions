...
public class CmdCreate extends FCommand
{
	public CmdCreate()
	{
		...
		this.requiredArgs.add("gang tag");
	...
	public void perform()
	{
		...
		if (fme.hasFaction())
		{
			msg("<b>You must leave your current Gang first.");
			return;
		}
		...
		if ( ! payForCommand(Conf.econCostCreate, "to create a new gang", "for creating a new gang")) return;
		...
		if (faction == null)
		{
			msg("<b>There was an internal error while trying to create your gang. Please try again.");
			return;
		}
	...
	// finish setting up the FPlayer
		...
		{
			follower.msg("%s<i> created a new gang %s", fme.describeTo(follower, true), faction.getTag(follower));
		}
		
		msg("<i>You should now: %s", p.cmdBase.cmdDescription.getUseageTemplate());

		if (Conf.logFactionCreate)
			P.p.log(fme.getName()+" created a new gang: "+tag);
	}
