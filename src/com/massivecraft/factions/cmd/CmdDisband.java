...
public class CmdDisband extends FCommand
{
	...
	public void perform()
	{
		...
		Faction faction = this.argAsFaction(0, fme == null ? null : myGang);
		...
		{
			msg("<i>This gang is designated as permanent, so you cannot disband it.");
			return;
		}
		...
		for (FPlayer fplayer : FPlayers.i.getOnline())
		{
			...
			{
				fplayer.msg("<h>%s<i> disbanded your gang.", who);
			}
			else
			{
				fplayer.msg("<h>%s<i> disbanded the gang %s.", who, faction.getTag(fplayer));
			}
		}
		if (Conf.logFactionDisband)
			P.p.log("The gang "+faction.getTag()+" ("+faction.getId()+") was disbanded by "+(senderIsConsole ? "console command" : fme.getName())+".");
		...
		if (Econ.shouldBeUsed() && ! senderIsConsole)
		{
			...
			if (amount > 0.0)
			{
				...
				msg("<i>You have been given the disbanded gang's bank, totaling %s.", amountString);
